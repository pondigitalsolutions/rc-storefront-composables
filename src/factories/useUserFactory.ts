...
export type UseUserFactoryParams<
    ...
    GetCountriesParams extends DefaultSearchParams
    > = {
        user: Ref<USER>;
        userorders: Ref<USERORDER[]>;
        countries: Ref<COUNTRY[]>;
        loadUser: (searchParams: UserParams) => Promise<SingleResult<USER>>;
        updateUser: (
        ...
        getCountries: (params: GetCountriesParams) => Promise<SearchResult<COUNTRY>>;
    };
...
>(
    ...
    return function useUser(cacheId: string): UseUser<USER> {
        ...
        const countries: Ref<COUNTRY[]> = ref(initialState ?.countries || []);
        const loading = ref(false);

        const load = async (params: USER_PARAMS) => {
            loading.value = true;
            const { data } = await factoryParams.loadUser(params);
            user.value = data;
        ...
        const update = async (params: UPDATE_USER_PARAMS) => {
            loading.value = true;
            const { user: mutationResult } = await factoryParams.updateUser(params);
        ...
        const loadCountries = async (params: COUNTRIES_PARAMS) => {
            loading.value = true;
            const { data: apiCountries } = await factoryParams.getCountries(params);
            countries.value = apiCountries;

            saveToInitialState({ countries: apiCountries });
            loading.value = false;
        };
